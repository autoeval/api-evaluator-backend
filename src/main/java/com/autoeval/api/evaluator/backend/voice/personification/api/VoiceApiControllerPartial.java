...
@RestController
@RequestMapping("/api/partial")
public class VoiceApiControllerPartial {
    private static final Logger LOGGER = LoggerFactory.getLogger(VoiceApiControllerPartial.class);

    @Value("classpath:responses_partial.json")
    ...
    public ResponseEntity<VoiceApiResponse> analyzeVoice(@RequestPart(value = "file")MultipartFile file/*, @RequestPart(value = "field1", required = false) String field1*/) throws IOException {
        ...
        LOGGER.info("Response for {} is {}", fileName, response);
